...
def main():
    ...
    region_id_ctr = conn.execute(
        "SELECT COALESCE(MAX(region_id),0) FROM brain_regions"
    ).fetchone()[0]
    ct_id_ctr = conn.execute(
        "SELECT COALESCE(MAX(cell_type_id),0) FROM cell_types"
    ).fetchone()[0]
    cell_id_ctr = conn.execute("SELECT COALESCE(MAX(cell_id),0) FROM cells").fetchone()[
        0
    ]
    oligo_id_ctr = conn.execute(
        "SELECT COALESCE(MAX(oligomer_id),0) FROM oligomers"
    ).fetchone()[0]
    ...
    for row in conn.execute(
        "SELECT patient_id, brain_region, region_id FROM brain_regions"
    ).fetchall():
        region_map[(row[0], row[1])] = row[2]
    for row in conn.execute(
        "SELECT region_id, cell_type, cell_type_id FROM cell_types"
    ).fetchall():
        ct_map[(row[0], row[1])] = row[2]
    for row in conn.execute(
        "SELECT image_filename, cell_id_in_image, cell_id FROM cells"
    ).fetchall():
    ...
    for region, cell_type in targets:
        for condition in ("HC", "PD"):
            ...
            min_vox = (
                int(RE_MINCELL.search(p0_files[0]).group(1))
                if RE_MINCELL.search(p0_files[0])
                else None
            )
            max_vox = (
                int(RE_MAXCELL.search(p0_files[0]).group(1))
                if RE_MAXCELL.search(p0_files[0])
                else None
            )
            ...
            if "cell_id_in_image" not in cdf.columns:
                print(
                    f"  WARNING missing cell_id_in_image in {cell_csv} — run run_cell_analysis_for_db.py first"
                )
                continue
            ...
            for row in cdf.to_dicts():
                ...
                rows_cells.append(
                    (
                        ...
                        (
                            int(row["n_puncta_in_cell"])
                            if row["n_puncta_in_cell"] is not None
                            else None
                        ),
                        conc,
                        img_fn,
    ...
    for region, cell_type in targets:
        for condition in ("HC", "PD"):
            oligo_csv = os.path.join(
                ...
                region,
                cell_type,
                f"{condition}_analysis",
                ANALYSIS_SUBDIR,
                "oligomers_in_cells_percentile_0.csv",
            )
            ...
            if n_existing_ic > 0:
                print(
                    f"  SKIP already loaded {n_existing_ic:,} in-cell for {region}/{cell_type}/{condition}"
                )
                continue
            ...
            for row in odf.to_dicts():
                ...
                batch.append(
                    (
                        ...
                        row.get("bg_per_punctum"),
                        row.get("bg_per_pixel"),
                        row.get("zi"),
                        row.get("zf"),
                        img_fn,
                    )
            ...
            inserted = conn.execute(
                "SELECT COUNT(*) FROM oligomers WHERE in_cell=TRUE"
            ).fetchone()[0]
            print(
                f"  {region}/{cell_type}/{condition}: done (skipped {skipped} rows with no patient ID)"
            )
    ...
    for region, cell_type in targets:
        for condition in ("HC", "PD"):
            ...
            if n_existing > 0:
                print(
                    f"  SKIP already loaded {n_existing:,} out-of-cell for {region}/{cell_type}/{condition}"
                )
                continue
            ...
            print(
                f"  {region}/{cell_type}/{condition}: streaming {spot_csv} (incell={'yes' if has_incell else 'no'}) ..."
            )
            ...
            conn.execute(
                "CREATE OR REPLACE TEMP TABLE pid_ct (patient_id VARCHAR, cell_type_id INTEGER)"
            )
            ...
            conn.commit()
            n_oc = conn.execute(
                "SELECT COUNT(*) FROM oligomers WHERE in_cell=FALSE AND brain_region=? AND cell_type=?",
                [region, cell_type],
            ).fetchone()[0]
    ...
    print("\n[5] Creating indexes ...")
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_cells_patient ON cells(patient_id, brain_region, cell_type)"
    )
    conn.execute("CREATE INDEX IF NOT EXISTS idx_oligo_cell   ON oligomers(cell_id)")
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_oligo_patient ON oligomers(patient_id, brain_region, cell_type, in_cell)"
    )
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_oligo_intensity ON oligomers(sum_intensity_in_photons)"
    )
    conn.commit()
    ...
    oc = conn.execute("SELECT COUNT(*) FROM oligomers WHERE in_cell=FALSE").fetchone()[
        0
    ]
