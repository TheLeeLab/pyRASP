...
class filters(object):
    ...
    def adjustable(self, w, h, kmax):
        ...
        beta = (
            ...
            / 2
            * (kmax / w0 - 1)
            / (np.arccos(1 - 2 * h) + np.pi / 2 * (kmax / w0 - 1))
        )
        T = (1 - beta) / w0 / 2
        return beta, T
...
def segoptim(u0seg, varseg, gainseg, otfmask, alpha, iterationN, ind):
    ...
    outi = optimize.minimize(
        calcost,
        x0,
        args=(u0i, vari, gaini, otfmask, alpha),
        method="L-BFGS-B",
        options=opts,
    )
...
def optimf(u0, varseg, gainseg, otfmask, Rs, R, alpha, iterationN):
    ...
    results = [
        pool.apply_async(
            segoptim, args=(u0seg, varseg, gainseg, otfmask, alpha, iterationN, ind)
        )
        for ind in range(Ns * Ns)
    ]
